...
class ChatRepository private constructor(context: Context) {
    ...
    suspend fun fetchTokenFromServer(userId: String, name: String, image: String = ""): String = withContext(Dispatchers.IO) {
        ...
        val url = buildString {
            ...
            if (name.isNotBlank()) {
                append("&name=")
                append(java.net.URLEncoder.encode(name, Charsets.UTF_8))
            }
            if (image.isNotBlank()) {
                append("&image=")
                append(java.net.URLEncoder.encode(image, Charsets.UTF_8))
            }
        }
    ...
    @RequiresApi(Build.VERSION_CODES.TIRAMISU)
    suspend fun fetchTokenWithSupabaseAuth(supabaseAccessToken: String, name: String = "", image: String = ""): String = withContext(Dispatchers.IO) {
        val baseUrl = appContext.getString(R.string.backend_base_url).trimEnd('/')
        val url = buildString {
    ...
    @RequiresApi(Build.VERSION_CODES.TIRAMISU)
    suspend fun fetchTokenWithFirebaseAuth(firebaseIdToken: String, userId: String, name: String = "", image: String = ""): String = withContext(Dispatchers.IO) {
        val baseUrl = appContext.getString(R.string.backend_base_url).trimEnd('/')
        val url = buildString {
            append(baseUrl).append("/token")
            val params = mutableListOf<String>()
